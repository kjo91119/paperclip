...
export function IssueDetail() {
  ...
  const rawDescriptionInitializedIssueIdRef = useRef<string | null>(null);
  const panelInitializedIssueIdRef = useRef<string | null>(null);
  ...
  const isOrchestratedMeeting = issue ? isIssueDetailReadOnly(issue) : false;
  const issueBrief = useMemo(
    () => buildIssueBrief({ title: issue?.title ?? "", description: issue?.description }),
  ...
  useEffect(() => {
    if (!issue) return;
    if (rawDescriptionInitializedIssueIdRef.current === issue.id) return;
    rawDescriptionInitializedIssueIdRef.current = issue.id;
    setRawDescriptionOpen(!(issueBrief.hasStructuredSections && !isMeetingReadingSurface));
  }, [issue?.id, issueBrief.hasStructuredSections, isMeetingReadingSurface]);

  useEffect(() => {
    if (!issue) return;
    if (panelInitializedIssueIdRef.current === issue.id) return;
    panelInitializedIssueIdRef.current = issue.id;
    if (!isMeetingReadingSurface && issueBrief.hasStructuredSections) {
      setPanelVisible(false);
    }
  }, [isMeetingReadingSurface, issue?.id, issueBrief.hasStructuredSections, setPanelVisible]);
  ...
  return (
    <div className={cn(isMeetingReadingSurface ? "max-w-6xl" : "max-w-[72rem]", "space-y-6")}>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          <span className="text-xs font-mono text-muted-foreground/40 shrink-0">
            작업 #{issue.issueNumber ?? issue.identifier ?? issue.id.slice(0, 8)}
          </span>
        ...
        <div className="rounded-2xl border border-border bg-card/80 p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-xl border border-border/80 bg-background/70 p-4">
              <div className="text-xs font-semibold text-muted-foreground">쉽게 말하면</div>
              <p className="mt-2 text-sm leading-6 text-foreground">
                {issueBrief.summary || "이 문서의 핵심 설명이 아직 적혀 있지 않습니다."}
              </p>
            </div>

            <div className="rounded-xl border border-border/80 bg-background/70 p-4">
              <div className="text-xs font-semibold text-muted-foreground">지금 결정할 것</div>
              {issueBrief.decisionItems.length > 0 ? (
                <ul className="mt-2 space-y-2 text-sm leading-6 text-foreground">
                  {issueBrief.decisionItems.map((item) => (
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  아직 구체적인 결정 항목이 정리되지 않았습니다. 코멘트로 방향만 짧게 정해도 충분합니다.
                </p>
              )}
            </div>

            {issueBrief.backgroundItems.length > 0 ? (
              <div className="rounded-xl border border-border/80 bg-background/70 p-4">
                <div className="text-xs font-semibold text-muted-foreground">참고 배경</div>
                <ul className="mt-2 space-y-2 text-sm leading-6 text-foreground">
                  {issueBrief.backgroundItems.map((item) => (
                    <li key={item} className="flex gap-2">
            ...
            {issueBrief.constraintItems.length > 0 ? (
              <div className="rounded-xl border border-border/80 bg-background/70 p-4">
                <div className="text-xs font-semibold text-muted-foreground">주의할 점</div>
                <ul className="mt-2 space-y-2 text-sm leading-6 text-foreground">
                  {issueBrief.constraintItems.map((item) => (
                    <li key={item} className="flex gap-2">
      ...
      <Collapsible
        open={rawDescriptionOpen}
        onOpenChange={setRawDescriptionOpen}
        className="rounded-2xl border border-border bg-card/70"
      >
        <CollapsibleTrigger className="flex w-full items-center justify-between px-4 py-3 text-left">
          <div>
            <div className="text-sm font-semibold text-foreground">
              {isMeetingReadingSurface ? "원문 회의 설명" : "원문 작업 지시"}
            </div>
            <div className="text-xs text-muted-foreground">
              원래 적힌 내용은 그대로 유지하고, 필요할 때만 펼쳐서 수정합니다.
            </div>
          </div>
          <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform", rawDescriptionOpen && "rotate-180")} />
        </CollapsibleTrigger>
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="comments" className="gap-1.5">
            ...
            {issue.meetingMode === "orchestrated" ? "회의 대화" : "대화"}
          </TabsTrigger>
              ...
              {childIssues.map((child) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="font-mono text-[11px] text-muted-foreground/45 shrink-0">
                      작업 #{child.issueNumber ?? child.identifier ?? child.id.slice(0, 8)}
                    </span>
                  </div>
