...
export function ApprovalDetail() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "승인하지 못했습니다"),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "거절하지 못했습니다"),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "수정 요청을 보내지 못했습니다"),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "재제출 처리하지 못했습니다"),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "댓글을 등록하지 못했습니다"),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "삭제하지 못했습니다"),
  });

  if (isLoading) return <PageSkeleton variant="detail" />;
  if (!approval) return <p className="text-sm text-muted-foreground">승인 요청을 찾을 수 없습니다.</p>;
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">승인이 완료되었습니다</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  요청한 에이전트에게 승인 결과와 연결된 이슈 검토 필요 사항이 전달되었습니다.
                </p>
              </div>
            </div>
            <Button
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              연결된 이슈는 요청한 에이전트가 후속 조치를 하고 닫을 때까지 계속 열려 있습니다.
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              이 예산 중단은 <Link to="/costs" className="underline underline-offset-2">/costs</Link>의 예산 제어 화면에서 해소하세요.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("거절된 이 에이전트를 삭제할까요? 이 작업은 되돌릴 수 없습니다.")) return;
